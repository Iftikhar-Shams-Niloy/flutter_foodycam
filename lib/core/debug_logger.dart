class DebugLogger {
  ...
  static void topPredictions(List<String> labels, List<double> probabilities, int count) {
    ...
    for (int i = 0; i < count && i < sortedIndices.length; i++) {
      ...
      log("Top ${i + 1}: ${labels[idx]} (${(probabilities[idx] * 100).toStringAsFixed(2)}%)");
    }
  }

  static void tensorInfo(String name, dynamic shape, dynamic type, [dynamic index]) {
    if (index != null) {
